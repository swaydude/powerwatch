...
final class ProducerCoroutine<E> extends kotlinx.coroutines.channels.ChannelCoroutine<E> implements kotlinx.coroutines.channels.ProducerScope<E> {
    ...
    public ProducerCoroutine(kotlin.coroutines.CoroutineContext parentContext, kotlinx.coroutines.channels.Channel<E> channel) {
    ...
    public void onCompletionInternal$kotlinx_coroutines_core(java.lang.Object state, int mode, boolean suppressed) {
        ...
        boolean zClose = get_channel().close(th);
        if (th == null || zClose || !suppressed) {
            return;
        }
        kotlinx.coroutines.CoroutineExceptionHandlerKt.handleExceptionViaHandler(getContext(), th);
    }
}
